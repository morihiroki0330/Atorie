...
void Dungeon_Stage::ThirdMapDataSet()
{
	for (int X = 0; X < M_SquareX; X++)
	{
		for (int Y = 0; Y < M_SquareY; Y++)
		{
			...
			{
				M_MapTexture[X][Y].Init("Assets/Sprite/Dungeon/Wall.DDS", M_SquareWide, M_SquareHeight);
			}else {
				if (M_MapData[X][Y] == GROUND)
				{
					M_MapTexture[X][Y].Init("Assets/Sprite/Dungeon/Ground.DDS", M_SquareWide, M_SquareHeight);
				}else {
					if (M_MapData[X][Y] == GROUNDFORMAL)
					{
						M_MapTexture[X][Y].Init("Assets/Sprite/Dungeon/Goal.DDS", M_SquareWide, M_SquareHeight);
					}
				}
			}
		}
